...
export const SpeculativeRulesScript = component$(() => {
  ...
  for (const config of state) {
    ...
    if ('urls' in config) {
      const urls = new Set(map[config.type].list[eagerness]?.urls || []);
      for (const url of config.urls) urls.add(url);
      map[config.type].list[eagerness] ||= { urls: [] };
      map[config.type].list[eagerness]!.urls = Array.from(urls);
    } else {
  ...
  for (const type in map) {
    for (const source in map[type as RuleType]) {
      for (const eagerness in map[type as RuleType][source as RuleSource]) {
        ...
        else rules[type as RuleType]!.push({ ...rule, eagerness: eagerness as Eagerness });
      }
    }
  }
  if (!rules.prefetch && !rules.prerender) return;
  return <script type="speculationrules" dangerouslySetInnerHTML={JSON.stringify(rules)} />
})